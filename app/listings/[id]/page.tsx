...
/** Load all products from the static JSON file */
  ...
  try {
    ...
    return products.map((p: any, i: number) => ({
      ...
      images:      Array.isArray(p.images) && p.images.length
                     ? p.images
                     : [p.thumbnail || "https://images.unsplash.com/photo-1560343090-f0409e92791a?w=400&q=80"],
      isFeatured:  i % 5 === 0,
      rating:      p.rating  || 0,
      stock:       p.stock   || 1,
      brand:       p.brand   || "",
...
/** Fetch a single product — tries backend first for Mongo IDs, falls back to JSON */
  ...
  if (isMongoId(id)) {
    try {
      ...
      if (res.ok) {
        ...
        if (data.success && data.listing) return data.listing;
      }
    } catch {
      // backend unreachable — fall through to JSON
    }
  }

  // ── Static JSON product (numeric id like "1", "2" …) ──
  const products = loadJsonProducts();
  return products.find((p: any) => p._id === id) || null;
}

/** Get related products — same source as the product itself */
...
export default async function ProductDetailPage({ params }: Props) {
  ...
  const images: string[] = Array.isArray(product.images) && product.images.length
    ? product.images
    : [product.thumbnail || "https://images.unsplash.com/photo-1560343090-f0409e92791a?w=400&q=80"];
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <p className="font-medium capitalize">{product.category.replace(/-/g, " ")}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium">{product.stock ?? 1} units</p>
              </div>
            </div>
          </div>
        ...
        <div className="w-full lg:w-80 flex-shrink-0">
          <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4 sticky top-24">
            ...
            <p className="text-sm text-gray-500 mb-4">
              {(product.stock ?? 1) > 0
                ? <span className="text-green-600 font-medium">In Stock ({product.stock ?? 1} available)</span>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-gray-800">{product.seller?.name || "Seller"}</p>
                <p className="text-xs text-gray-500 capitalize">
                  {product.category.replace(/-/g, " ")} Seller
                </p>
              </div>
            </div>
          </div>
      ...
      {related.length > 0 && (
        <section className="mt-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {related.map((p: any) => (
              <ProductCard key={p._id} listing={p} />
            ))}
          </div>
        </section>
      )}
